...
test('the stub should be usable as client', coreMarbles(({ expect }) => {
...
test('the stub should select based method', coreMarbles(({ expect }) => {
  type Response = { a: number }
  type Response2 = { b: number }
  type Endpoint = RestEndpoint<{
    method: 'GET'
...
test('the stub should select based on pathname', coreMarbles(({ expect }) => {
  type Response = { a: number }
  type Response2 = { b: number }
  type Endpoint = RestEndpoint<{
    method: 'GET'
...
test('the stub should select based on search', coreMarbles(({ expect }) => {
  type Response = { a: number }
  type Response2 = { b: number }
  type Endpoint = RestEndpoint<{
    method: 'GET'
...
test('the stub should select based on headers', coreMarbles(({ expect }) => {
  type Response = { a: number }
  type Response2 = { b: number }
  type Endpoint = RestEndpoint<{
    method: 'GET'
  ...
  const client = createStubRestClient<Api, typeof endpoints>(endpoints)
  const search: Record<'a', string> = { a: '1' } as const
  const headers: Record<'header-1', 'value-2'> = { 'header-1': 'value-2' } as const
  const response = client({
    method: 'GET',
    pathname: '/path',
    search,
...
test('the stub respect the delay', coreMarbles(({ expect }) => {
  type Response = { a: number }
  type Endpoint = RestEndpoint<{
    method: 'GET'
    pathname: '/path'
